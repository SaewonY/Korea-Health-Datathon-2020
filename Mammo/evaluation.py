...
def evaluation_metrics (label, pred):
    ...
    return metrics, np.round(np.mean(list(metrics.values())), 4)
...
def test_label_loader(root_path):
    labels = {}
...
if __name__ == '__main__':
    ...
    label_dict = test_label_loader(os.path.join(DATASET_PATH,'test'))
    ...
    _, score = evaluation_metrics(labels, preds)
    print(score)